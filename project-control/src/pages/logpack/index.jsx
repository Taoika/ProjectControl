...
export default function LogPack() {
  ...
  const getData = () => {
    ...
    React.axios('get', 'http://39.98.41.126:31113/apiError/serverPackage', setLoad, '')
      .then(res => {
        let data = [];
        res.map(i => {
          data.push({
  ...
  const getExpandData = (expanded, record) => {
    if (expanded && expandeddata[record.packageName] === undefined) {
      ...
      React.axios('post', 'http://39.98.41.126:31113/apiError/serverMethod', setLoad1, '', { packageName: record.packageName })
        .then(res => {
