...
export default function LogPack() {
  ...
  const getData = () => {
    ...
    React.axios('get', 'http://106.13.18.48/monitor/apiError/serverPackage', setLoad, '')
      .then(res => {
        let data = [];
        res.map(i => {
          data.push({
  ...
  const getExpandData = (expanded, record) => {
    if (expanded && expandeddata[record.packageName] === undefined) {
      ...
      React.axios('post', 'http://106.13.18.48/monitor/apiError/serverMethod', setLoad1, '', { packageName: record.packageName })
        .then(res => {
