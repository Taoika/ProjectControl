...
export default function MyDrawer() {
    ...
    const agreeOrrefuse = (type, applicationId) => {
        ...
        React.axios('post', 'http://39.98.41.126:31113/application/update', setLoad, '', { applicationId, number: type }, '', '', getData).then(
        )
    }
    const getData = () => {
        setData([])
        setLoad({ left: '10.2895vw', top: '5.75vw' })
        React.axios('get', 'http://39.98.41.126:31113/message/watch', setLoad).then(
            res => {
                setData(res)
            },
        )
    ...
    const getData2 = () => {
        setData2([])
        setLoad({ left: '10.2895vw', top: '5.75vw' })
        React.axios('get', 'http://39.98.41.126:31113/application/MySend', setLoad).then(
            res => {
                console.log(res);
                setData2(res)
            },
    ...
    const readed = (applicationId) => {
        setLoad({ left: '10.2895vw', top: '5.75vw' })
        React.axios('post', 'http://39.98.41.126:31113/application/deleteMySend', setLoad, '', { applicationId }, '', '', getData2).then(
        )
    }
    return (
        <>
